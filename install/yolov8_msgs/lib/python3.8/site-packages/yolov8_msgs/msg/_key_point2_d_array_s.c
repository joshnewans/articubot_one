...
bool yolov8_msgs__msg__key_point2_d_array__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("yolov8_msgs.msg._key_point2_d_array.KeyPoint2DArray", full_classname_dest, 51) == 0);
  }
  yolov8_msgs__msg__KeyPoint2DArray * ros_message = _ros_message;
  {  // data
    ...
    if (!yolov8_msgs__msg__KeyPoint2D__Sequence__init(&(ros_message->data), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create yolov8_msgs__msg__KeyPoint2D__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    yolov8_msgs__msg__KeyPoint2D * dest = ros_message->data.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!yolov8_msgs__msg__key_point2_d__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * yolov8_msgs__msg__key_point2_d_array__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("yolov8_msgs.msg._key_point2_d_array");
  ...
  {  // data
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = yolov8_msgs__msg__key_point2_d__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
